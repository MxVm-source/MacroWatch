...
def _build_auto_plan(symbol: str, verdict: dict, structure: dict):
    ...
    margin_pct = GATE_MARGIN_PCT_A if grade == "A" else GATE_MARGIN_PCT_B
    try:
    ...
    capital = equity * margin_pct / 100
    size    = round(capital * GATE_LEV / entry, GATE_SIZE_DEC)
    warnings.append(f"sized at {margin_pct:.0f}% margin of ${equity:,.0f} equity "
                    f"(${capital:,.0f} margin) — {grade}-grade")
